...
df = pd.read_csv("/faststorage/project/Maesa/steps/HybPip/mega353_complete/output/seq_lengths.txt", sep="\t")
df = df.drop(0)
...
for fn in os.listdir(): #list file in current directory
	...
	if locus in df.columns:
		...
		# first sample
		sp = df_red["Species"][0]
		exon1 = list(SeqIO.parse("/faststorage/project/Maesa/steps/HybPip/mega353_complete/"+sp+"/"+locus+"/"+sp+"/sequences/FNA/"+locus+".FNA", "fasta"))[0]
		exon1.id = "exon1"
		# second sample
		sp = df_red["Species"][1]
		exon2 = list(SeqIO.parse("/faststorage/project/Maesa/steps/HybPip/mega353_complete/"+sp+"/"+locus+"/"+sp+"/sequences/FNA/"+locus+".FNA", "fasta"))[0]
		exon2.id = "exon2"
